  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
      <div className="relative bg-white pt-10 pb-5 px-5 sm:px-8 md:px-12 lg:px-16 xl:px-20 rounded-2xl w-11/12 text-center">
        <CloseIcon className="absolute top-4 right-4 h-6 cursor-pointer stroke-2" onClick={onClose} />
        ...
        <div className="flex justify-around">
          {moodOptions.map((mood) => (
            <span
              key={mood}
              className={`cursor-pointer text-4xl ${selectedMood === mood ? "transform scale-125 transition-transform duration-100 ease-in delay-250" : ""}`}
        ...
        <button className="bg-cyan-400 text-white text-sm font-bold mt-4 px-6 py-3 rounded cursor-pointer" onClick={onSubmit}>
          Confirm
        </button>
      </div>
    </div>
